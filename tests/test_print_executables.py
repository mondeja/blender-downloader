...
@pytest.mark.parametrize("create_executables", (True, False))
def test_print_executables(
    operative_system,
    print_blender_executable,
    print_python_executable,
    create_executables,  # if not create executables and should be printed -> error
):
    kwargs = dict(create_executables=create_executables)
    ...
    with create_temporary_directory_by_os(operative_system, **kwargs) as dir_filepath:
        ...
        with contextlib.redirect_stdout(mocked_stdout), contextlib.redirect_stderr(
            mocked_stderr
        ):

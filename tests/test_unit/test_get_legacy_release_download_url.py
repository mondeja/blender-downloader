...
from testing_helpers import SUPPORTED_EXTENSIONS_FOR_EXTRACTION
...
def test_get_legacy_release_download_url(
    blender_version, operative_system, bits, arch,
):
    blender_Version = BlenderVersion(blender_version)

    if blender_Version < BlenderVersion(MINIMUM_BLENDER_VERSION_SUPPPORTED):
        mocked_stderr = io.StringIO()
        with pytest.raises(SystemExit), contextlib.redirect_stderr(
            mocked_stderr,
        ):
            get_legacy_release_download_url(
                blender_version, operative_system, bits, arch,
    ...
    url = get_legacy_release_download_url(
        blender_version, operative_system, bits, arch)
    ...
    def assert_url(url_end_schema):
        ...
        assert url == (
            f'{expected_url_start}{major_minor_blender_version}/'
            f'blender-{url_end}'
        )
    ...
    if operative_system == 'macos':
        if blender_Version >= BlenderVersion('2.93'):
            ...
            #: BlenderVersion("2.71") < blender_Version < BlenderVersion("2.79")
            assert_url('{blender_version}-OSX_10.6-i386.zip')
        else:
            assert_url('{blender_version}-OSX_10.6-x86_64.zip')
        ...
        if blender_Version >= BlenderVersion('4.3') and arch == 'arm64':
            ...
            assert_url('{blender_version}-windows{bits}.zip')
    else:  # operative_system == "linux":  # noqa: PLR5501
        if blender_Version >= BlenderVersion('2.93') or (
            blender_Version >= BlenderVersion('2.83.14') and
            blender_Version < BlenderVersion('2.84')
        ):
            ...
            #: BlenderVersion("2.64") < blender_Version < BlenderVersion("2.79")  # noqa: E501
            if bits == 32:
